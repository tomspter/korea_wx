...
async function naver({type, query, display, start, sort}) {
    let url = ''
    if (type === 'news') {
        url = `https://openapi.naver.com/v1/search/news.json?query=${encodeURI(query)}&display=${display}&start=${start}&sort=${sort}`
    } else if (type === 'book') {
        url = `https://openapi.naver.com/v1/search/book.json?query=${encodeURI(query)}&display=${display}&start=${start}`
    } else if (type=== 'movie'){
        url = `https://openapi.naver.com/v1/search/book.json?query=${encodeURI(query)}&display=${display}&start=${start}&country=KR`
    }
